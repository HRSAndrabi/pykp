...
found. If knowing all optimal solutions is important, consider using the
...
Example:
    ...
    `solve` method with "mzn_gecode" as the `method` argument::
...
def mzn_gecode(items: np.ndarray[Item], capacity: int) -> Arrangement:
    """
    Solves the knapsack problem using the minizinc and gecode solver. This
    solver is not robust to multiple solutions.
